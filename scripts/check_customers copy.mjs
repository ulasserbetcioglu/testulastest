...
async function check() {
    console.log('Fetching customers schema...')
    const { data, error } = await supabase.from('customers').select('*').limit(1)
    if (error) {
        console.error('Error:', error)
    } else {
        if (data.length > 0) {
            console.log('Columns found in customers:', Object.keys(data[0]))
        } else {
            console.log('Table is empty, cannot easily infer columns from data. Trying query info...');
        }
    }
}
