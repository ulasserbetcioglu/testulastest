...
function getPestsString(pests: string[] | string | null): string {
  ...
  if (Array.isArray(pests)) {
    ...
    if (pests.startsWith('{') && pests.endsWith('}')) {
      ...
      pestList = cleaned.split(',').map(p => p.trim().replace(/^"|"$/g, ''));
    }
    // JSON array format: ["item1","item2"]
    else if (pests.trim().startsWith('[') && pests.trim().endsWith(']')) {
      try {
...
export function generateContractHtml({ proposal, settings, contractNumber, startDate: customStart, endDate: customEnd }: ContractParams): string {
  ...
  const serviceRows = serviceItems.map(item => {
    ...
    let freqSummer = `Aylık ${item.visit_count} Ziyaret`;
    let freqWinter = `Aylık ${item.visit_count} Ziyaret`;

    // If it's the main periodic service (not a one-off), use global frequencies
    if (item.unit_type !== 'seferlik' && !isObs) {
      freqSummer = `Aylık ${summerFreq} Ziyaret`;
      freqWinter = `Aylık ${winterFreq} Ziyaret`;
    } else if (item.unit_type === 'seferlik') {
      freqSummer = 'Tek Seferlik';
      freqWinter = 'Tek Seferlik';
    }
    ...
    return `<tr>
      ...
      <td style="${S.td}">${freqSummer}${suffix}</td>
      <td style="${S.td}">${freqWinter}${suffix}</td>
  ...
  <div class="contract-body" style="padding: 10px 40px 30px 40px;">
    ...
    <table style="width:100%; border-collapse:collapse; table-layout: fixed;">
      <thead><tr>
        ...
        <th style="${S.th} width: 20%;">ZARARLI TÜRÜ</th>
        <th style="${S.th} text-align:center; width: 15%;">HİZMET MODELİ<br/>PERİYODİK</th>
        <th style="${S.th} width: 15%;">YAZ SIKLIĞI<br/>(Nisan-Eylül)</th>
        <th style="${S.th} width: 15%;">KIŞ SIKLIĞI<br/>(Ekim-Mart)</th>
        <th style="${S.th} width: 15%;">UYGULAMA ALANI</th>
      </tr></thead>
    ...
    <table style="width:100%; border-collapse:collapse; margin:8px 0;">
      <thead><tr>
        ...
        <th style="${S.th} width:45%;">HİZMET ADI</th>
        <th style="${S.th} width:25%;">BİRİM</th>
        <th style="${S.th} width:25%; text-align:right;">BİRİM FİYAT</th>
      </tr></thead>
      <tbody>
        ${serviceItems.map((item, idx) => {
    const unitRaw = item.unit_type || 'aylık';
    const unitLabel = unitRaw === 'seferlik' ? 'SEFER BAŞI' : 'AYLIK';
    return `<tr>
            <td style="${S.td} text-align:center;">${idx + 1}</td>
            <td style="${S.td}">${item.service_name.toUpperCase()}</td>
            <td style="${S.td} text-align:center;">${unitLabel}</td>
            <td style="${S.td} text-align:right; white-space:nowrap;">${formatTL(item.unit_price)}.-TL+KDV</td>
          </tr>`;
  }).join('')}
         <tr style="background-color: #f0f9ff;">
          <td colspan="3" style="${S.td} text-align:right; font-weight:700;">TOPLAM HİZMET BEDELİ (KDV HARİÇ):</td>
          <td style="${S.td} text-align:right; font-weight:700; white-space:nowrap;">${formatTL(perVisitTotal)}.-TL</td>
        </tr>
      </tbody>
    </table>
    
    <p style="${S.sub}"><strong>Yazı ile:</strong> ${amountWords} TÜRKLİRASI + KDV</p>
    <p style="${S.sub}"><strong>9.1.1.</strong> Hizmet Kalemi: 3. YAPILACAK İŞİN TANIMI MADDESİNE BAKINIZ.</p>
    <p style="${S.sub}"><strong>9.1.2.</strong> Kapsam: 3. YAPILACAK İŞİN TANIMI MADDESİNE BAKINIZ.</p>
    ${materialSection}
