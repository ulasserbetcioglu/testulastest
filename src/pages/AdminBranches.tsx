...
import { Search, Plus, Edit2, Trash2, Building, MapPin, Layout, ArrowRight, Brain } from 'lucide-react';
...
const AdminBranches: React.FC = () => {
  ...
  const [aiBranchTarget, setAiBranchTarget] = useState<{ id: string; name: string } | null>(null);
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto min-h-screen">
                ...
                <tr key={branch.id} className="group hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-3 px-2 align-top text-right">
                    <div className="flex items-center justify-end gap-3">
                      <button 
                        onClick={() => setAiBranchTarget({ id: branch.customer_id, name: branch.customer?.kisa_isim || 'Müşteri' })}
      ...
      {aiBranchTarget && (
        <CustomerAiAnalysisModal
          customerId={aiBranchTarget.id}
          customerName={aiBranchTarget.name}
          onClose={() => setAiBranchTarget(null)}
        />
      )}
    </div>
  );
