        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
              currentValue === true ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            Evet / Var
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${
              currentValue === false ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
            }`}
          >
            Hayır / Yok
          </button>
                  ...
                  <div className="p-4">
                    {item.equipment.properties && Object.keys(item.equipment.properties).length > 0 ? (
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {Object.entries(item.equipment.properties).map(([key, config]) => (
                          <div key={key}>
