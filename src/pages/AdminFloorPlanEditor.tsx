...
const AdminFloorPlanEditor: React.FC = () => {
  ...
  // --- RESİM BOYUTLARINI DOĞRULAMA (EKLENDİ) ---
  // Mevcut kayıtlı bir plan açıldığında, resim boyutları veritabanında yoksa veya hatalıysa
  // resmin doğal boyutlarını alıp state'i günceller.
  useEffect(() => {
    if (activeFloor?.background) {
      const img = new Image();
      img.src = activeFloor.background;
      img.onload = () => {
        // Eğer kayıtlı boyut ile gerçek boyut farklıysa güncelle
        if (activeFloor.width !== img.naturalWidth || activeFloor.height !== img.naturalHeight) {
          setFloors(prevFloors => {
            const newFloors = [...prevFloors];
            // Döngüye girmemesi için sadece gerçekten fark varsa güncelle
            if (newFloors[activeFloorIndex].width !== img.naturalWidth) {
  ...
  }, [activeFloor?.background, activeFloorIndex]);
  ...
  const handleCustomerChange = async (e: React.ChangeEvent<HTMLSelectElement>, preselectedBranchId?: string) => {
    ...
    if (!preselectedBranchId) setSelectedBranch('');
    ...
    if (data && data.length > 0) {
      ...
      if (mainPlan.floors && mainPlan.floors.length > 0) {
        ...
        setFloors([{
          ...
          width: mainPlan.width || 1000, // Bu değer useEffect ile düzeltilecek
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    if ((isDraggingElement || isResizingElement) && selectedElementId) {
        // Ölçeği hesaba katarak doğru hareket miktarı
              ...
              >
                ...
                {activeFloor.background ? (
                  <image 
                    ...
                    opacity={0.9}
                    // Resim olduğu gibi kalır, esneme yok
                  />
                ) : (
