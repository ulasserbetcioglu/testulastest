...
const BranchTrendAnalysis: React.FC<BranchTrendAnalysisProps> = ({ branchId: propBranchId, branchName: propBranchName }) => {
  const [branchId, setBranchId] = useState<string>(propBranchId || '');
  const [currentBranchName, setCurrentBranchName] = useState<string>(propBranchName || '');
  ...
  // Self-fetch logic if props are missing
  useEffect(() => {
    const initData = async () => {
      if (propBranchId) {
        setBranchId(propBranchId);
        setCurrentBranchName(propBranchName || '');
        return;
      }

      // Attempt to load from localAuth/Supabase if no props
      try {
        const localSession = localAuth.getSession();
        if (localSession && localSession.type === 'branch') {
          setBranchId(localSession.id);
          setCurrentBranchName(localSession.name);
          return;
        }

        const { data: { user } } = await supabase.auth.getUser();
        if (user) {
          const { data: branchData } = await supabase.from('branches').select('id, sube_adi').eq('auth_id', user.id).maybeSingle();
          if (branchData) {
            setBranchId(branchData.id);
            setCurrentBranchName(branchData.sube_adi);
          }
        }
      } catch (err) {
