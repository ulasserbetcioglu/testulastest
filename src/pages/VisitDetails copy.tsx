...
import FlyAnalysisModal from '../components/FlyAnalysisModal';
import { Plus, Trash, MapPin, Navigation, Mail, PenTool as Tool, Camera, X as CloseIcon, ChevronDown, ChevronUp, Image as ImageIcon, Eye, EyeOff, FileText, CheckCircle2, AlertCircle } from 'lucide-react';
import { calculateDistance } from '../lib/utils';
import { sendEmail, getRecipientEmails } from '../lib/emailClient';
import { toast } from 'sonner';
import { generateBiocidalVisitPdf } from '../utils/biocidalVisitPdfGenerator';
...
const ParasutConfirmationModal: React.FC<{
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (sendToParasut: boolean) => void;
  data: {
    serviceInvoice?: { amount: number; description: string };
    materialInvoice?: { amount: number; items: any[] };
  };
  loading: boolean;
}> = ({ isOpen, onClose, onConfirm, data, loading }) => {
  if (!isOpen) return null;

  const total = (data.serviceInvoice?.amount || 0) + (data.materialInvoice?.amount || 0);
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
        <div className="p-6">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Paraşüt Fatura Onayı</h2>
              <p className="text-sm text-gray-500">Otomatik fatura taslakları oluşturulacak</p>
            </div>
          </div>
          ...
          <div className="space-y-4 mb-8">
            {data.serviceInvoice && (
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
                <div className="flex justify-between items-start mb-1">
                  <span className="text-sm font-semibold text-blue-800">Hizmet Faturası</span>
                  <span className="font-bold text-blue-900">{data.serviceInvoice.amount.toLocaleString('tr-TR')} TL</span>
                </div>
                <p className="text-xs text-blue-600 leading-relaxed">{data.serviceInvoice.description}</p>
                <div className="mt-2 flex items-center gap-1.5 text-[10px] font-medium text-blue-500 uppercase tracking-wider">
                  <CheckCircle2 size={12} /> Hizmet Bedeli Uygulanacak
                </div>
              </div>
            )}
            ...
            {data.materialInvoice && (
              <div className="bg-green-50 border border-green-100 rounded-xl p-4">
                <div className="flex justify-between items-start mb-1">
                  <span className="text-sm font-semibold text-green-800">Malzeme Faturası</span>
                  <span className="font-bold text-green-900">{data.materialInvoice.amount.toLocaleString('tr-TR')} TL</span>
                </div>
                <div className="space-y-1">
                  {data.materialInvoice.items.map((item: any, i: number) => (
            ...
            <div className="pt-4 border-t border-gray-100 flex justify-between items-center">
              <span className="text-base font-bold text-gray-700">Toplam Tutarı</span>
              <span className="text-2xl font-black text-gray-900">{total.toLocaleString('tr-TR')} TL</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-3">
            <button
              ...
              disabled={loading}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-blue-200 transition-all active:scale-[0.98]"
            >
              {loading ? (
                <div className="w-6 h-6 border-3 border-white/30 border-t-white rounded-full animate-spin"></div>
              ) : (
                <>Paraşüt'e Aktar ve Bitir</>
              )}
            </button>
            <button
...
const VisitDetails: React.FC = () => {
  ...
  const [showFlyAnalysisModal, setShowFlyAnalysisModal] = useState(false);
  const [selectedFlyAnalysisTarget, setSelectedFlyAnalysisTarget] = useState<{ equipmentId: string, propertyKey: string } | null>(null);

  // Paraşüt States
  const [showParasutModal, setShowParasutModal] = useState(false);
  const [parasutLoading, setParasutLoading] = useState(false);
  const [parasutInfo, setParasutInfo] = useState<{
    serviceInvoice?: { amount: number; description: string };
    materialInvoice?: { amount: number; items: any[] };
  }>({});
  ...
  const fetchPaidProducts = async () => {
    try {
      const { data } = await supabase.from('paid_products').select('id, name, unit_type, price, parasut_id').eq('is_active', true).order('name');
  ...
  const fetchVisitDetails = async () => {
    try {
      ...
      if (data) {
        // Supabase join results can sometimes be returned as arrays depending on PK/FK definitions in client.
        // We map them to the expected single object structure if needed.
        ...
        if (currentOpId) {
          ...
          if (prevVisit) {
            mappedData.previous_visit_id = prevVisit.id;
            mappedData.previous_visit = prevVisit;
          }
        }
        setVisit(mappedData as Visit);
        setIsEditMode(mappedData.status === 'completed');

        if (mappedData.equipment_checks) setEquipmentChecks(mappedData.equipment_checks);
        if (mappedData.pest_types) setSelectedPestTypes(mappedData.pest_types);

        if (mappedData.visit_type) {
          const types = Array.isArray(mappedData.visit_type) ? mappedData.visit_type : [mappedData.visit_type];
          setSelectedVisitTypes(types);
          setShowPaidVisitAmount(types.includes('ucretli'));
        }

        if (mappedData.notes) {
          const match = mappedData.notes.match(/Ücretli ziyaret tutarı: (\d+) TL/);
          if (match) {
            setPaidVisitAmount(match[1]);
            setNotes(mappedData.notes.replace(/Ücretli ziyaret tutarı: \d+ TL\n\n/, ''));
          } else {
            setNotes(mappedData.notes);
          }
        }

        if (mappedData.report_number) setReportNumber(mappedData.report_number);
        if (mappedData.report_photo_url) setReportPhotoPreview(mappedData.report_photo_url);
  ...
  const fetchPreviousPaidMaterials = async (visitId: string) => {
    ...
    if (data && data.items) {
      ...
      setPreviousPaidMaterials(mappedItems);
  ...
  // --- FLY ANALYSIS HANDLERS ---

  const isFlyControlDevice = (name: string) => {
    const n = name.toLowerCase();
    return n.includes('efc') || n.includes('efk') || n.includes('sinek') || n.includes('fly');
  };

  const openFlyAnalysis = (equipmentId: string, propertyKey: string) => {
    setSelectedFlyAnalysisTarget({ equipmentId, propertyKey });
    setShowFlyAnalysisModal(true);
  };

  const handleFlyAnalysisComplete = (results: { total: number; details: Record<string, number> }) => {
    if (!selectedFlyAnalysisTarget) return;

    const { equipmentId } = selectedFlyAnalysisTarget;

    const equipmentItem = branchEquipment.find(item => item.id === equipmentId);
    if (!equipmentItem || !equipmentItem.equipment.properties) return;

    let changesMade = false;
    const newChecks = { ...equipmentChecks };
    if (!newChecks[equipmentId]) newChecks[equipmentId] = {};

    // Helper to fuzzy match property labels
    const findKeyForLabel = (labelKeywords: string[]) => {
      return Object.entries(equipmentItem.equipment.properties || {}).find(([, prop]) => {
        const label = prop.label.toLowerCase();
        return labelKeywords.some(kw => label.includes(kw));
      })?.[0];
    };

    // Map analysis categories to potential property keywords
    const mapping: Record<string, string[]> = {
      'house_fly': ['karasinek', 'kara sinek'],
      'fruit_fly': ['meyve', 'meyve sineği', 'küçük'],
      'drain_fly': ['gider', 'gider sineği', 'lağım'],
      'pantry_pest': ['ambar', 'güve'],
      'large': ['arı', 'diğer', 'böcek']
    };

    // Update specific fields based on mapping
    Object.entries(results.details).forEach(([typeId, count]) => {
      const keywords = mapping[typeId];
      if (keywords) {
        const key = findKeyForLabel(keywords);
        if (key) {
          newChecks[equipmentId][key] = count;
          changesMade = true;
        }
      }
    });

    // Also update the clicked field if it wasn't updated by the mapping (fallback to total or mapped value)
    const clickedKey = selectedFlyAnalysisTarget.propertyKey;
    if (!changesMade) {
      newChecks[equipmentId][clickedKey] = results.total;
      changesMade = true;
    }

    if (changesMade) {
      setEquipmentChecks(newChecks);
      toast.success('Analiz sonuçları ilgili alanlara dağıtıldı.');
    }
  };
  ...
  const saveVisit = async () => {
    ...
    setLoading(true);
    try {
      // 1. Önce plan ve tutar hesaplaması yapalım (Sadece yeni tamamlanan ziyaretler için)
      if (!isEditMode) {
        const pInfo = await calculateInvoiceInfo();
        if (pInfo.serviceInvoice || pInfo.materialInvoice) {
          setParasutInfo(pInfo);
          setShowParasutModal(true);
          setLoading(false);
          return; // Modal açıldığı için burada duruyoruz, save ana işlemi Modal on Confirm'de olacak
        }
      }

      await executeSaveVisit();
  ...
  const calculateInvoiceInfo = async () => {
    let serviceInvoice: any = null;
    let materialInvoice: any = null;

    // 1. Hizmet Faturası Planlaması
    // ÖNCELİK A: Manuel Ücretli Ziyaret (Eğer 'ucretli' seçiliyse ve tutar girildiyse)
    if (selectedVisitTypes.includes('ucretli') && paidVisitAmount && parseFloat(paidVisitAmount) > 0) {
      serviceInvoice = {
        amount: parseFloat(paidVisitAmount),
        description: `Ücretli Zararlı Mücadelesi Hizmeti`
      };
    }
    // ÖNCELİK B: Sözleşmeli Fiyatlandırma (Şube veya Müşteri Bazlı)
    else if (visit?.branch?.id) {
      // Önce şube fiyatlandırmasını çek
      let { data: pricing }: { data: any } = await supabase
        .from('branch_pricing')
        .select('monthly_price, per_visit_price, branch_id')
        .eq('branch_id', visit.branch.id)
        .maybeSingle();

      // Şube fiyatı yoksa Müşteri fiyatına bak (Fallback)
      if (!pricing && visit.customer?.id) {
        const { data: cPricing } = await supabase
          .from('customer_pricing')
          .select('monthly_price, per_visit_price, customer_id')
          .eq('customer_id', visit.customer.id)
          .maybeSingle();
        pricing = cPricing;
      }

      if (pricing) {
        // Sefer Başı Bedel (Eğer tanımlıysa her ziyarette çıkar)
        if (pricing.per_visit_price && pricing.per_visit_price > 0) {
          serviceInvoice = {
            amount: pricing.per_visit_price,
            description: `Zararlı Mücadelesi (Sefer Başı)`
          };
        }
        // Aylık Bedel (Eğer sefer başı yoksa ve ayın son planlı ziyaretiyse)
        else if (pricing.monthly_price && pricing.monthly_price > 0) {
          const visitDate = new Date(visit.visit_date);
          const monthStr = visitDate.toISOString().slice(0, 7);
          const startOfMonth = `${monthStr}-01`;
          const nextMonthDate = new Date(visitDate.getFullYear(), visitDate.getMonth() + 1, 1);
          const startOfNextMonth = nextMonthDate.toISOString().split('T')[0];

          // 1. Kapsamı Belirle (Şube mi Müşteri mi?)
          const isBranchBased = !!pricing.branch_id;

          const visitQuery = supabase.from('visits').select('id, status');
          if (isBranchBased) {
            visitQuery.eq('branch_id', visit.branch.id);
          } else {
            visitQuery.eq('customer_id', visit.customer.id);
          }

          const { data: monthlyVisits } = await visitQuery
            .gte('visit_date', startOfMonth)
            .lt('visit_date', startOfNextMonth);

          const visitIds = monthlyVisits?.map(v => v.id) || [];

          // 2. MÜKERRER KONTROLÜ: Kapsamdaki herhangi bir ziyaret için zaten 'monthly_plan' faturası kesildi mi?
          const { data: existingMonthlyInvoice } = await supabase
            .from('parasut_invoices')
            .select('id')
            .eq('source_type', 'monthly_plan')
            .in('source_id', visitIds)
            .maybeSingle();

          if (!existingMonthlyInvoice) {
            // 3. PLAN (SCHEDULE) KONTROLÜ
            ...
            // Schedule yoksa varsayılan olarak 1 ziyaret kuralı (ayı boş geçmemek için)
            const requiredVisits = schedule ? (schedule.visits_required || 0) : 1;
            const completedCount = monthlyVisits?.filter(v => v.status === 'completed' && v.id !== visit.id).length || 0;

            if (completedCount + 1 >= requiredVisits) {
              serviceInvoice = {
                amount: pricing.monthly_price,
                description: `Zararlı Mücadelesi (Aylık Periyodik)`
              };
            }
          }
        }
    ...
    // Malzeme Faturası Kontrolü
    const validPaidItems = paidProductUsage.filter(i => i.productId && i.quantity && parseFloat(i.quantity) > 0);
    if (validPaidItems.length > 0) {
      let totalAmount = 0;
      const items = validPaidItems.map(item => {
        const p = paidProducts.find(x => x.id === item.productId);
        ...
        return {
          ...
          parasut_product_id: (p as any)?.parasut_id // SQL ile eklemiştik
        };
      });
      materialInvoice = { amount: totalAmount, items };
    }

    return { serviceInvoice, materialInvoice };
  };

  const executeSaveVisit = async (sendToParasut: boolean = false) => {
    try {
      ...
      // Paraşüt Aktarımı
      if (sendToParasut && visit) {
        setParasutLoading(true);
        try {
          // Cari Paraşüt ID çek (Önce şube, sonra müşteri fallback)
          let pContactId: string | null = null;
          const { data: bData } = await supabase.from('branches').select('parasut_id').eq('id', visit.branch?.id).maybeSingle();
          pContactId = bData?.parasut_id;

          let hasWithholding = false;
          if (!pContactId && visit.customer?.id) {
            const { data: cData } = await supabase.from('customers').select('parasut_id, has_withholding').eq('id', visit.customer.id).maybeSingle();
            pContactId = cData?.parasut_id;
            hasWithholding = !!cData?.has_withholding;
          } else if (visit.customer?.id) {
            // Already have pContactId from branches, but still need withholding flag from customer
            const { data: cData } = await supabase.from('customers').select('has_withholding').eq('id', visit.customer.id).maybeSingle();
            hasWithholding = !!cData?.has_withholding;
          }

          if (!pContactId) throw new Error('Bu şube veya müşteri için Paraşüt müşteri eşleşmesi (Parasut ID) bulunamadı.');

          // v2.15: Müşteri bakiyesini çek
          // Şirket ayarlarını çek (Fatura notu için)
          const { data: cSettings } = await supabase
            .from('company_settings')
            .select('parasut_invoice_note')
            .eq('id', 1)
            .single();

          let currentBalance = 0;
          try {
            const { data: balanceData } = await supabase.functions.invoke('parasut-fetch', {
              body: { type: 'contact_show', contact_id: String(pContactId) }
            });
            if (balanceData?.success && balanceData?.data) {
              currentBalance = balanceData.data.balance || 0;
            }
          } catch (balErr) {
            console.warn('Bakiye çekilemedi:', balErr);
          }

          const visitMonth = new Date(visit.visit_date).toLocaleDateString('tr-TR', { month: 'long', year: 'numeric' });
          const baseDescription = `${visitMonth} - ${visit.branch?.sube_adi}`;

          const bankAccountId = '1000353860'; // Garanti BBVA ID from list_accounts

          // v2.16: Konsolide Fatura Kalemleri Hazırla
          const consolidatedItems: any[] = [];
          let combinedTitle = '';

          // Hizmet Kalemi
          if (parasutInfo.serviceInvoice) {
            consolidatedItems.push({
              name: 'Zararlı Mücadelesi',
              unit_price: parasutInfo.serviceInvoice.amount,
              quantity: 1,
              parasut_product_id: '1030118145',
              vat_rate: 20
            });
            combinedTitle = 'Zararlı Mücadelesi Hizmeti';
          }

          // Malzeme Kalemleri
          if (parasutInfo.materialInvoice) {
            const materialList = parasutInfo.materialInvoice.items.map(item => `${item.name} (${item.quantity} adet)`).join(', ');
            parasutInfo.materialInvoice.items.forEach(item => {
              consolidatedItems.push({
                name: item.name,
                unit_price: item.unit_price,
                quantity: item.quantity,
                parasut_product_id: item.parasut_product_id,
                vat_rate: 20
              });
            });

            if (combinedTitle) {
              combinedTitle += ' ve Malzeme Satışı';
            } else {
              combinedTitle = 'Malzeme Satışı';
            }

            // Malzeme listesini başlığın sonuna ekle
            combinedTitle += ` - ${materialList}`;
          }

          if (consolidatedItems.length > 0) {
            const balanceColor = currentBalance > 0 ? ' (BORÇLU)' : (currentBalance < 0 ? ' (ALACAKLI)' : '');
            const balanceStr = `${currentBalance.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} TL${balanceColor}`;
            
            // Sabit notu ayarlardan al veya varsayılanı kullan
            let customIbanNote = cSettings?.parasut_invoice_note || 'MÜŞTERİ GÜNCEL BAKİYESİ: {BALANCE}\n\nÖDEME BİLGİLERİ:\nBanka: Garanti BBVA\nIBAN: TR66 0006 2000 0370 0006 2027 89';
            customIbanNote = customIbanNote.replace('{BALANCE}', balanceStr);

            const finalTitle = `${baseDescription} - ${combinedTitle} (Rapor No: ${reportNumber})`;
            const finalNote = `${finalTitle}\n\n${customIbanNote}`;

            const { error: invoiceError } = await supabase.functions.invoke('parasut-automation', {
              body: {
                source_type: parasutInfo.serviceInvoice ? 'monthly_plan' : 'material_sale',
                source_id: id,
            ...
            if (invoiceError) throw new Error(`Paraşüt aktarım hatası: ${invoiceError.message || 'Bilinmeyen hata'}`);
          }
          toast.success('Faturalar Paraşüt\'e başarıyla aktarıldı.');
        } catch (pErr: any) {
          console.error('Paraşüt Aktarım Hatası:', pErr);
          toast.warning('Ziyaret kaydedildi ancak Paraşüt aktarımı başarısız oldu: ' + pErr.message);
        } finally {
          setParasutLoading(false);
          setShowParasutModal(false);
        }
      }
  ...
  const handleGenerateEk1Report = async () => {
    try {
      setLoading(true);
      ...
      // Biyosidal ürün detaylarını çek
      const usedBiocidalItems = await Promise.all(
        biocidalUsage
          .filter(u => u.productId)
          .map(async (u) => {
            // Daha detaylı ürün bilgisi için (aktif madde, ruhsat no vb.) fetch gerekebilir
            // Zaten biocidal_products tablosundaki (name, active_ingredient, license_number, manufacturer) alanlarına sahibiz (biocidalProducts state'inde)
            const fullProduct: any = biocidalProducts.find(p => p.id === u.productId) || {};
            
            return {
              name: fullProduct.name || 'Bilinmeyen Ürün',
              licenseNumber: (fullProduct as any).license_number || '-',
              manufacturer: (fullProduct as any).manufacturer || '-',
              activeIngredient: (fullProduct as any).active_ingredient || '-',
              batchNumber: '', // İlerde UI'dan alınabilir
              quantity: u.quantity,
              dosage: u.dosage,
              unit: u.unit,
              applicationMethod: 'Pulverizasyon / Jel'
            };
          })
      );

      await generateBiocidalVisitPdf({
        visitDate: new Date(visit?.visit_date || '').toLocaleDateString('tr-TR'),
        startTime: startTime,
        endTime: endTime,
        customerName: visit?.customer.kisa_isim || visit?.customer.cari_isim || '',
        customerAddress: visit?.customer.id ? (await supabase.from('customers').select('adres').eq('id', visit.customer.id).single()).data?.adres : '',
        branchName: visit?.branch?.sube_adi,
        operatorName: operatorData?.name || 'Bilinmeyen Operatör',
        operatorCertificate: (operatorData as any)?.certificate_no || '-',
        responsibleManager: companySettings?.responsible_manager || '-',
        pestTypes: selectedPestTypes.map(id => pestTypes.find(p => p.id === id)?.label || id),
        notes: explanation || notes,
        biocidalUsage: usedBiocidalItems,
      ...
      toast.success('EK-1 Raporu başarıyla oluşturuldu.');
    } catch (err: any) {
      console.error(err);
      toast.error('Rapor oluşturulurken hata: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!visit) return <div className="p-8 text-center text-gray-500">Ziyaret bulunamadı</div>;
    ...
    <div className="max-w-4xl mx-auto p-3 sm:p-6 space-y-6 pb-24">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <div className="text-sm text-gray-500 flex justify-between sm:justify-start sm:gap-4">
        ...
        <button
          onClick={handleGenerateEk1Report}
          className="flex items-center justify-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 transition-all shadow-md shadow-blue-100"
        >
          <FileText size={20} />
          EK-1 RAPORU (PDF)
        </button>
                        ...
                        <div key={item.id} className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm">
                          <div className="flex justify-between items-start mb-3 border-b border-gray-100 pb-2">
                            ...
                            <div className="flex items-center gap-2">
                              {isFlyControlDevice(item.equipment.name) && (
                                <button
                                  type="button"
                                  onClick={() => {
                                    const defaultKey = Object.entries(item.equipment.properties || {}).find(([_, p]) => p.type === 'number')?.[0];
                                    if (defaultKey) openFlyAnalysis(item.id, defaultKey);
                                    else toast.error('Bu ekipmanda sayısal değer girilecek alan yok.');
                                  }}
                                  className="p-1.5 bg-blue-50 text-blue-600 rounded-lg border border-blue-200 hover:bg-blue-100 transition-colors"
                                  title="Sinek Analizi Yap"
                                >
                                  <Camera size={20} />
                                </button>
                              )}
                                  ...
                                  {prop.type === 'boolean' ? (
                                    <div className="flex bg-gray-100 p-0.5 sm:p-1 rounded-lg">
                                      <button
                                        ...
                                        className={`px-2.5 sm:px-3 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all ${(equipmentChecks[item.id]?.[key] === 'false' || !equipmentChecks[item.id]?.[key])
                                      ...
                                      <button
                                        ...
                                        className={`px-2.5 sm:px-3 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all ${equipmentChecks[item.id]?.[key] === 'true'
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                  ...
                  <div>
                    ...
                    <input type="text" className="w-full border border-gray-300 rounded-lg p-2 sm:p-2.5 text-sm" value={item.unit} onChange={(e) => handleBiocidalChange(idx, 'unit', e.target.value)} placeholder="Doz" />
                  </div>
                  <div>
      ...
      {showFlyAnalysisModal && (
        <FlyAnalysisModal
          isOpen={showFlyAnalysisModal}
          onClose={() => setShowFlyAnalysisModal(false)}
          onAnalysisComplete={handleFlyAnalysisComplete}
        />
      )}

      <ParasutConfirmationModal
        isOpen={showParasutModal}
        onClose={() => setShowParasutModal(false)}
        onConfirm={(toParasut) => executeSaveVisit(toParasut)}
        data={parasutInfo}
