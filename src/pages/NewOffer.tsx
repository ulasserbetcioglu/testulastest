...
const NewOfferPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ]).then(() => setScriptsLoaded(true)).catch(_ => toast.error("Gerekli kütüphaneler yüklenemedi."));
  }, []);
  ...
  // --- REVISION LOGIC (v2.27) --- // Removed revision logic
  // useEffect(() => {
  //   const fetchSourceProposal = async () => {
  ...
  //         .from('offer_items')
  //         .select('*')
  //         .eq('offer_id', revisionOf)
  //         .order('line_number', { ascending: true });

  //       if (itemsError) throw itemsError;

  //       // 3. Pre-fill state
  //       setSelectedCustomerId(sourceOffer.customer_id);
  //       setSelectedBranchId(sourceOffer.branch_id || '');
  //       setAciklama(sourceOffer.aciklama || '');
  ...
  //       toast.info(`${sourceOffer.teklif_no} numaralı teklif revizyon için yüklendi.`);
  //     } catch (err: any) {
