  ...
  return (
    <div className="space-y-4 max-w-full overflow-x-hidden">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 px-2">
        <h1 className="text-xl sm:text-2xl font-bold text-gray-800">ŞUBE ZİYARET TAKVİMİ</h1>
                ...
                <div className="space-y-0.5 sm:space-y-1 overflow-y-auto max-h-[60px] sm:max-h-[100px]">
                  {dayVisits.map(visit => (
                    <div
                      ...
                      title={`Müşteri: ${visit.customer.kisa_isim}`}
                    >
                      <span className="hidden sm:inline">
                        {visit.customer.kisa_isim.length > 15 
                          ? `${visit.customer.kisa_isim.substring(0, 15)}...` 
                          : visit.customer.kisa_isim}
                      </span>
                      <span className="sm:hidden">
                        {getStatusText(visit.status)} - {visit.customer.kisa_isim.substring(0, 8)}
                        {visit.customer.kisa_isim.length > 8 ? '..' : ''}
                      </span>
                    </div>
                  ))}
                </div>
