...
import { 
  ...
  CheckCircle, Clock, X, MapPin, Loader2 
} from 'lucide-react';
...
import FloorPlanViewer from '../components/Branches/FloorPlanViewer'; // Kroki bileşeni
...
// --- ALT BİLEŞENLER (Müşteri Panelinden Uyarlandı) ---
...
// 4. Pestisit Kullanımı (Placeholder)
const BranchPesticideUsageView = ({ branchId }: { branchId: string }) => (
  <div className="p-6 text-center text-gray-500 bg-white border border-dashed border-gray-300 rounded-lg">
    ...
    <p>Bu şube için pestisit kullanım raporu verisi bulunamadı.</p>
  </div>
);

// 5. Trend Analizi Grafikleri
...
const BranchDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState('visits'); // Varsayılan sekme
  ...
  return (
    <div className="space-y-6 p-4 md:p-6 bg-gray-50 min-h-screen">
      ...
      {/* DETAYLI SEKMELER (Müşteri Paneli Gibi) */}
      {branchId && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="bg-gray-50 px-4 py-2 border-b border-gray-200 flex flex-wrap gap-2">
                <button onClick={() => setActiveTab('visits')} className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'visits' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:bg-white'}`}><Calendar size={16} /> Ziyaretler</button>
                ...
                <button onClick={() => setActiveTab('pesticides')} className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'pesticides' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:bg-white'}`}><Bug size={16} /> Pestisit</button>
            ...
            <div className="p-6 bg-gray-50/30 min-h-[300px]">
                ...
                {activeTab === 'pesticides' && <BranchPesticideUsageView branchId={branchId} />}
