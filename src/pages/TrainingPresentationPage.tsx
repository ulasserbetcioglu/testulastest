...
import { 
  ...
  Maximize, Upload, Check, X, Award, ImageIcon, 
  Settings
} from 'lucide-react';
...
import PresentationAiModal from '../components/Training/PresentationAiModal';
import { Sparkles, RefreshCw as RefreshIcon } from 'lucide-react';
...
const TrainingPresentationPage = () => {
  ...
  const handleAiGenerate = (generatedSlides: Slide[]) => {
    const newPresentation: Presentation = {
      id: '',
      name: 'AI Tarafından Oluşturulan Sunum',
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] gap-4">
        <RefreshIcon className="animate-spin text-green-600" size={32} />
      ...
      {activeView === 'list' && (
        <>
          <div className="flex justify-between items-center mb-8">
            ...
            <div className="flex gap-2">
               ...
               <button 
                ...
                className="bg-purple-600 text-white px-4 py-2 rounded hover:bg-purple-700 text-sm font-bold flex items-center gap-2 shadow-lg shadow-purple-100 transition-all hover:scale-105 active:scale-95"
               >
                 <Sparkles size={16} /> AI ile Hazırla
               </button>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <button 
              onClick={() => setShowAiModal(true)}
              className="border-2 border-dashed border-purple-200 bg-purple-50/30 rounded-xl flex flex-col items-center justify-center h-full min-h-[200px] hover:border-purple-400 hover:bg-purple-50 transition-all group text-purple-600"
            >
                <div className="w-10 h-10 rounded-full bg-purple-100 flex items-center justify-center group-hover:bg-purple-200 mb-2">
                    <Sparkles className="text-purple-600" />
                </div>
                <span className="text-sm font-bold uppercase tracking-wider group-hover:text-purple-700">AI ile Hazırla</span>
                <span className="text-[10px] text-purple-400 mt-1">Sadece Bir Konu Girin</span>
            </button>
          </div>
