...
const CustomerTrendAnalysis: React.FC = () => {
  ...
  const fetchEquipmentTypeActivities = async (branchIds: string[]) => {
    if (branchIds.length === 0) { 
      ...
      console.log('❌ Branch IDs boş, ekipman verileri çekilemiyor');
      return;
    }

    try {
      console.log('🔄 Ekipman verileri çekiliyor...', { branchIds });

      // Ekipman verilerini çek
      ...
      if (eqError) {
        console.error('❌ Ekipman verileri hatası:', eqError);
        toast.error('Ekipman verileri yüklenemedi');
        return;
      }

      console.log('✅ Ekipman Verileri:', equipmentData?.length || 0, 'adet');
      console.log('📦 İlk 3 Ekipman:', equipmentData?.slice(0, 3));

      // Ziyaret verilerini çek
      ...
      if (visitError) {
        console.error('❌ Ziyaret verileri hatası:', visitError);
        return;
      }

      console.log('✅ Ziyaret Verileri:', visitsData?.length || 0, 'adet');
      console.log('📦 İlk 3 Equipment Check:', visitsData?.slice(0, 3).map(v => v.equipment_checks));

      if (!equipmentData || equipmentData.length === 0) {
        console.warn('⚠️ Ekipman verisi bulunamadı');
        setEquipmentTypeData([]);
        return;
      }

      if (!visitsData || visitsData.length === 0) {
        console.warn('⚠️ Ziyaret verisi bulunamadı');
        setEquipmentTypeData([]);
        return;
      }
      ...
      console.log('🗺️ Ekipman Map Boyutları:', {
        byCode: equipmentByCode.size,
        byId: equipmentById.size
      });
      ...
      visitsData.forEach(visit => {
        if (visit.equipment_checks && typeof visit.equipment_checks === 'object') {
          Object.entries(visit.equipment_checks).forEach(([key, checkData]: [string, any]) => {
            ...
            if (!equipment) {
                console.log(`⚠️ Tanımsız ekipman: ${key}`);
      ...
      console.log('📊 İşleme Özeti:', {
        processedChecks,
        activityMapSize: activityMap.size,
        missingEquipments: missingEquipments.size
      });
      ...
      console.log('👥 Gruplar:', Array.from(nameGroups.keys()));
      ...
      nameGroups.forEach((group, equipmentName) => {
        ...
        if (allFieldsInData.size === 0) {
          console.log(`⚠️ ${equipmentName}: Veri yok, atlanıyor`);
          return;
        }
        ...
        if (activities.length > 0) {
          console.log(`✅ ${equipmentName}: ${activities.length} aktivite eklendi`);
          resultData.push({
            type: equipmentName,
      ...
      console.log('🎯 Sonuç:', resultData.length, 'ekipman tipi');
      setEquipmentTypeData(resultData);

    } catch (error) {
      console.error('💥 Ekipman aktivite analizi hatası:', error);
      toast.error('Ekipman analizi başarısız');
    }
  };
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 text-gray-700 transition-colors disabled:opacity-50"
          >
