...
import { Search, Download, Upload, Edit2, Trash2, Plus, DollarSign, ToggleLeft, ToggleRight, Brain } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import type { Customer } from '../types';
import AddCustomerModal from '../components/Customers/AddCustomerModal';
import EditCustomerModal from '../components/Customers/EditCustomerModal';
import BulkPricingModal from '../components/Customers/BulkPricingModal';
import CustomerAiAnalysisModal from '../components/Customers/CustomerAiAnalysisModal';
...
const Customers: React.FC = () => {
  ...
  const [aiAnalysisTarget, setAiAnalysisTarget] = useState<Customer | null>(null);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {isAdmin && (
            <button
              ...
              className="px-3 py-1.5 bg-indigo-600 text-white rounded hover:bg-indigo-700 transition-colors flex items-center gap-1.5 text-sm"
            >
              <GitMerge size={16} />
              <span className="hidden sm:inline">Müşteri Birleştir</span>
              ...
              {filteredCustomers.map((customer) => (
                <tr key={customer.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-sm text-right space-x-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); setAiAnalysisTarget(customer); }}
                      className="text-purple-500 hover:text-purple-700 mr-1" title="AI Risk Analizi"
                    >
                      <Brain size={16} />
                    </button>
                    <button 
      ...
      {aiAnalysisTarget && (
        <CustomerAiAnalysisModal
          customerId={aiAnalysisTarget.id}
          customerName={aiAnalysisTarget.kisa_isim}
