...
const AdminMonthlyVisitSchedule = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const transformedSchedules = allSchedules.map(schedule => {
        const custData = Array.isArray(schedule.customers) ? schedule.customers[0] : schedule.customers;
        const branchData = Array.isArray(schedule.branches) ? schedule.branches[0] : schedule.branches;
        const opData = Array.isArray(schedule.operators) ? schedule.operators[0] : schedule.operators;

        let branchCustomerData: any = null;
        if (branchData?.customers) {
          branchCustomerData = Array.isArray(branchData.customers) ? branchData.customers[0] : branchData.customers;
        }
        ...
        return {
          ...
          customer: custData ? { kisa_isim: custData.kisa_isim } : null,
          branch: branchData ? {
            sube_adi: branchData.sube_adi,
            customer_id: branchData.customer_id,
            customer: branchCustomerData ? { kisa_isim: branchCustomerData.kisa_isim } : { kisa_isim: '' }
          } : null,
          operator: opData ? { name: opData.name } : null
        };
      });
      ...
      const transformedBranches = (branchesRes.data || []).map(branch => {
        const branchCustData = Array.isArray(branch.customers) ? branch.customers[0] : branch.customers;
        return {
          id: branch.id,
          sube_adi: branch.sube_adi,
          customer_id: branch.customer_id,
          customer: branchCustData ? { kisa_isim: branchCustData.kisa_isim } : { kisa_isim: '' }
        };
      });
  ...
  const getProcessedCustomers = () => {
    if (formData.selectedMonths.length === 0) return customers.map(c => ({ ...c, display: c.kisa_isim }));

    return customers.map(customer => {
      const customerBranches = branches.filter(b => b.customer_id === customer.id);

      // Eğer hiç şubesi yoksa, müşterinin kendisi bir birimdir
      if (customerBranches.length === 0) {
        ...
        return isScheduled ? null : { ...customer, display: customer.kisa_isim };
      }

      // Şubelerin kaçı planlı?
      ...
      const planlilar = customerBranches.filter(b => scheduledBranchIds.has(b.id)).length;
      const toplam = customerBranches.length;

      // Eğer tüm şubeler planlıysa bu müşteriyi gizle
      if (planlilar === toplam) return null;

      return {
        ...customer,
        display: `${customer.kisa_isim} (${planlilar}/${toplam} Planlı)`
      };
    }).filter(Boolean) as (Customer & { display: string })[];
  };
  ...
  const getAvailableBranches = () => {
    ...
    // Zaten planlı olanları tamamen gizle (User: "gözükmesin")
    return customerBranches.filter(branch => !scheduledBranchIds.has(branch.id));
  };
  ...
  const handleSave = async () => {
    try {
      // Validasyonlar
      if (formData.multiCustomerMode && formData.selectedCustomerIds.length === 0) { toast.error('Lütfen en az bir müşteri seçin'); return; }
      if (!formData.multiCustomerMode && formData.bulkMode && formData.selectedBranches.length === 0) { toast.error('Lütfen en az bir şube seçin'); return; }
      if (!formData.multiCustomerMode && !formData.bulkMode && formData.type === 'branch' && !formData.branch_id) { toast.error('Lütfen bir şube seçin'); return; }
      if (!formData.multiCustomerMode && !formData.bulkMode && formData.type === 'customer' && !formData.customer_id) { toast.error('Lütfen bir müşteri seçin'); return; }
      ...
      if (editingSchedule) {
        ...
        const records: any[] = [];
        if (formData.multiCustomerMode && formData.selectedCustomerIds.length > 0) {
          formData.selectedCustomerIds.forEach(custId => {
            const customerBranches = branches.filter(b => b.customer_id === custId);
            if (customerBranches.length > 0) {
              customerBranches.forEach(branch => {
                formData.selectedMonths.forEach(month => {
                  records.push({
                    customer_id: null, branch_id: branch.id, operator_id: formData.operator_id || null,
                    month, visits_required: formData.visits_required, year: formData.year, notes: formData.notes
                  });
                });
              });
            } else {
              // Şubesi yoksa direk müşteriye ata
              formData.selectedMonths.forEach(month => {
                records.push({
                  customer_id: custId, branch_id: null, operator_id: formData.operator_id || null,
                  month, visits_required: formData.visits_required, year: formData.year, notes: formData.notes
                });
              });
            }
          });
        } else if (formData.bulkMode && formData.selectedBranches.length > 0) {
  ...
  const filteredSchedules = useMemo(() => {
    return schedules.filter(schedule => {
      ...
      const matchesCustomer = !selectedCustomerFilter ||
        schedule.customer_id === selectedCustomerFilter ||
        (schedule.branch?.customer_id === selectedCustomerFilter);

      const matchesBranch = !selectedBranchFilter || schedule.branch_id === selectedBranchFilter;

      return matchesSearch && matchesOperator && matchesCustomer && matchesBranch;
    });
  }, [schedules, searchTerm, selectedOperatorFilter, selectedCustomerFilter, selectedBranchFilter]);
  ...
  const getUnscheduledBranchesForMonth = (month: number) => {
    ...
    return branches.filter(b => {
      ...
      const matchesCustomer = !selectedCustomerFilter || b.customer_id === selectedCustomerFilter;
      const matchesBranch = !selectedBranchFilter || b.id === selectedBranchFilter;

      return !scheduledBranchIds.has(b.id) && matchesSearch && matchesCustomer && matchesBranch;
    });
  };
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-4 bg-white p-4 rounded-lg shadow-sm border">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setSelectedBranchFilter(''); // Reset branch when customer changes
            }}
            className="px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
          >
          ...
          <select
            ...
            className="px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 disabled:bg-gray-50 disabled:text-gray-400"
          >
            <option value="">Tüm Şubeler</option>
            {branches
              .filter(b => b.customer_id === selectedCustomerFilter)
              .map(b => (
              ...
              <div className="bg-gray-50 px-4 py-3 border-b flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-2 self-end sm:self-auto">
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm transition-colors ${!isScheduledOpen
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-sm transition-colors ${isUnscheduledOpen
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              {/* Toplu Mod Checkbox */}
              {!editingSchedule && (
                <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.bulkMode && !formData.multiCustomerMode}
                      disabled={formData.multiCustomerMode}
                    ...
                    />
                    <div>
                      <span className="font-semibold text-blue-900 block">Toplu Şube Modu</span>
                      <span className="text-sm text-blue-700 italic">Tek müşteri, çok şube</span>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-3 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={formData.multiCustomerMode}
                      onChange={(e) => setFormData({
                        ...formData,
                        multiCustomerMode: e.target.checked,
                        bulkMode: false,
                        customer_id: '',
                        selectedCustomerIds: [],
                        branch_id: '',
                    ...
                    />
                    <div>
                      <span className="font-semibold text-blue-900 block font-black">Çoklu Müşteri Modu 🚀</span>
                      <span className="text-sm text-blue-700 italic">Ayda 1 olan işler için ideal</span>
                    </div>
                  </label>
                </div>
              )}
              ...
              <div className="grid gap-5">
                {/* Müşteri Seçimi */}
                {!formData.multiCustomerMode ? (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Müşteri</label>
                    <select
                      value={formData.customer_id}
                      onChange={(e) => setFormData({ ...formData, customer_id: e.target.value, branch_id: '', selectedBranches: [] })}
                      disabled={!!editingSchedule || formData.selectedMonths.length === 0}
                      className="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 py-2 disabled:bg-gray-100 disabled:text-gray-400"
                    >
                      <option value="">{formData.selectedMonths.length === 0 ? 'Önce Ay Seçiniz' : 'Müşteri Seçin'}</option>
                      {getProcessedCustomers().map(c => (
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="block text-sm font-medium text-gray-700">Müşteriler</label>
                      <div className="flex gap-2">
                        <button
                          type="button"
                          onClick={() => {
                            const filtered = getProcessedCustomers()
                              .filter(c => c.display.toLowerCase().includes(formData.customerSearch.toLowerCase()));
                            setFormData(p => ({ ...p, selectedCustomerIds: filtered.map(c => c.id) }));
                          }}
                        ...
                        <button
                          type="button"
                          onClick={() => setFormData(p => ({ ...p, selectedCustomerIds: [] }))}
                    ...
                    <div className="mb-2">
                      <div className="relative">
                        ...
                        <input
                          ...
                          onChange={(e) => setFormData(p => ({ ...p, customerSearch: e.target.value }))}
                          className="w-full pl-8 pr-3 py-1.5 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none"
                        />
                      </div>
                    </div>
                    <div className="border rounded-lg p-2 max-h-48 overflow-y-auto bg-gray-50">
                      {getProcessedCustomers()
                        .filter(c => c.display.toLowerCase().includes(formData.customerSearch.toLowerCase()))
                        .map(c => (
                        <label key={c.id} className="flex items-center p-2 hover:bg-white rounded cursor-pointer transition-colors group">
                          <input
                            type="checkbox"
                            checked={formData.selectedCustomerIds.includes(c.id)}
                            onChange={() => setFormData(p => ({
                              ...p,
                              selectedCustomerIds: p.selectedCustomerIds.includes(c.id)
                                ? p.selectedCustomerIds.filter(id => id !== c.id)
                                : [...p.selectedCustomerIds, c.id]
                            }))}
                            className="mr-3 w-4 h-4 rounded text-blue-600 focus:ring-blue-500 border-gray-300"
                          />
                          <span className="text-sm font-medium text-gray-700 group-hover:text-blue-600">{c.display}</span>
                        </label>
                      ))}
                    </div>
                  </div>
                      ...
                      {MONTH_NAMES.map((m, idx) => {
                        ...
                        return (
                          <div
                            ...
                            className={`text-center text-sm py-2 rounded cursor-pointer border transition-all ${isSelected ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-200 hover:border-blue-400'
                              }`}
                          >
                            {m}
                          </div>
                ...
                {/* Şubeler */}
                {formData.type === 'branch' && !formData.multiCustomerMode && (
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="block text-sm font-medium text-gray-700">
                        {formData.bulkMode ? 'Şubeler (Çoklu)' : 'Şube'}
                      </label>
                      {formData.bulkMode && availableBranches.length > 0 && (
                        <div className="flex gap-2">
                          <button
                            type="button"
                            onClick={() => setFormData(p => ({ ...p, selectedBranches: availableBranches.map(b => b.id) }))}
                            className="text-xs text-blue-600 font-medium hover:underline"
                          >
                            Tümünü Seç
                          </button>
                          ...
                          <button
                            type="button"
                            onClick={() => setFormData(p => ({ ...p, selectedBranches: [] }))}
                            className="text-xs text-red-600 font-medium hover:underline"
                          >
                            Temizle
                          </button>
                    ...
                    {formData.bulkMode ? (
                      <div className="border rounded-lg p-2 max-h-48 overflow-y-auto bg-gray-50">
                        {availableBranches.length === 0 ? (
                          <div className="text-center py-4 text-gray-500 text-sm italic">
                            {formData.customer_id ? 'Seçilen aylar için planlanacak şube kalmadı' : 'Lütfen önce müşteri seçin'}
                          </div>
                        ) : (
                          availableBranches.map((b: any) => (
                            <label key={b.id} className="flex items-center p-2 hover:bg-white rounded cursor-pointer transition-colors group">
                              <input
                                type="checkbox"
                                ...
                                className="mr-3 w-4 h-4 rounded text-blue-600 focus:ring-blue-500 border-gray-300"
                              />
                              <span className="text-sm font-medium text-gray-700 group-hover:text-blue-600">{b.sube_adi}</span>
                            </label>
                          ))
                        )}
                      </div>
                      ...
                      <select
                        ...
                        disabled={!!editingSchedule || !formData.customer_id}
                        className="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 py-2 disabled:bg-gray-100 disabled:text-gray-400"
                      >
                        <option value="">{formData.customer_id ? 'Şube Seçin' : 'Lütfen Müşteri Seçin'}</option>
                        {availableBranches.map((b: any) => (
                          <option key={b.id} value={b.id}>{b.sube_adi}</option>
                        ))}
                      </select>
