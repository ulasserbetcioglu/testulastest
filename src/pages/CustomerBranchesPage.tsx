...
import { 
  Building, MapPin, Phone, Mail, Package, Calendar, AlertCircle, FileText, Bug, Layout, TrendingUp, Loader2, Shield, Leaf, Brain, FileDown,
  RefreshCw, Download, Map as MapIcon, ChevronDown, ChevronUp, Eye, X, XCircle, CheckCircle, Clock, CreditCard, ChevronRight
} from 'lucide-react';
...
interface SaleItem {
  ...
  unit_price: number;
  total_price: number;
  products: { name: string, parasut_id?: number | null } | null;
}
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const mappedVisits = (visitData || []).map(v => ({
          ...v,
          operator: Array.isArray(v.operator) ? v.operator[0] : v.operator
        }));
        setVisits(mappedVisits as any);
    ...
    const fetchUsage = async () => {
      try {
        ...
        const { data, error } = await supabase
          ...
          .select(`id, quantity, unit, created_at, product:biocidal_products (name, active_ingredient, parasut_id), visit:visits (visit_date)`)
...
// --- 3.5. EKİPMAN HARİTASI (YENİ) ---
const BranchEquipmentMapView = ({ branchId, branchData }: { branchId: string; branchData: any }) => {
  const [equipments, setEquipments] = useState<any[]>([]);
  const [mapPositions, setMapPositions] = useState<any>({});
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        // Fetch floor plan to get map_equipment_positions
        const { data: plan } = await supabase
          .from('branch_floor_plans')
          .select('id, map_equipment_positions, background_url')
          .eq('branch_id', branchId)
          .eq('status', 'active')
          .maybeSingle();
        ...
        const { data: eqs } = await supabase
          .from('branch_equipments')
          .select('id, equipment_code, equipment:equipment_id (name, type)')
          .eq('branch_id', branchId);

        const mappedEqs = (eqs || []).map(eq => ({
          ...eq,
          equipment: Array.isArray(eq.equipment) ? eq.equipment[0] : eq.equipment
        }));
        setMapPositions(plan?.map_equipment_positions || {});
        setFloorPlanUrl(plan?.background_url || '');
        setEquipments(mappedEqs);
      } catch (error) {
  ...
  if (loading) return <div className="p-8 text-center text-gray-500">Harita yükleniyor...</div>;

  return (
    <div className="h-[600px] border rounded-xl overflow-hidden shadow-inner bg-gray-50 relative flex flex-col">
       <MapEquipmentPanel
          branchId={branchId}
          branchLat={branchData?.latitude}
          branchLng={branchData?.longitude}
          branchName={branchData?.sube_adi}
          equipments={equipments}
          mapPositions={mapPositions}
          floorPlanImageUrl={floorPlanUrl}
          readOnly={true}
          onMapPositionsChange={() => {}} // Read-only for customers
        />
    </div>
  );
};
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6"><div className="space-y-4">{sale.items.map((item, idx) => (<div key={idx} className="flex justify-between items-center py-2 border-b border-gray-100 last:border-0"><div><p className="font-medium text-gray-800">{item.products?.name || 'Bilinmeyen Ürün'}</p>{item.products?.parasut_id && <p className="text-[10px] font-mono text-blue-600 bg-blue-50 px-1 py-0.5 rounded inline-block">ID: {item.products.parasut_id}</p>}<p className="text-xs text-gray-500">{item.quantity} adet x {item.unit_price?.toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</p></div><div className="font-semibold text-gray-700">{(item.quantity * item.unit_price).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} ₺</div></div>))}</div></div>
...
const BranchMaterialUsageList = ({ branchId }: { branchId: string }) => {
  ...
  useEffect(() => {
    const fetchSales = async () => {
      try {
        const { data, error } = await supabase.from('paid_material_sales').select(`id, sale_date, total_amount, items:paid_material_sale_items (id, quantity, unit_price, products:paid_products ( name, parasut_id ))`).eq('branch_id', branchId).order('sale_date', { ascending: false }).limit(10);
        if (error) throw error;
...
// --- 6. BRANCH AI ANALİZ SEKMESİ ---
const BranchAiAnalysisView = ({ branch }: { branch: any }) => {
  ...
  const [visitStats, setVisitStats] = useState<{ count: number; pests: string[] }>({ count: 0, pests: [] });

  useEffect(() => {
    Promise.all([
      supabase.from('company_settings').select('company_name, logo_url, telefon, email, adres, website').single(),
      supabase.from('customers').select('kisa_isim, cari_isim, sehir, telefon, email').eq('id', branch.customer_id).single(),
      supabase.from('ai_branch_analyses')
        .select('*')
        .eq('branch_id', branch.id)
        .order('created_at', { ascending: false })
        .limit(1)
        .maybeSingle()
    ]).then(([settingsRes, customerRes, savedRes]) => {
      if (settingsRes.data) setCompanySettings(settingsRes.data);
      if (customerRes.data) setCustomer(customerRes.data);
      if (savedRes.data) {
        setSavedRecord(savedRes.data);
        setAnalysis(savedRes.data.analysis_text);
        setVisitStats({ count: savedRes.data.visit_count || 0, pests: savedRes.data.top_pests || [] });
      }
    });
  }, [branch.id, branch.customer_id]);
  ...
  const runAnalysis = async () => {
    ...
    setAnalysis('');
    try {
      // Fetch recent visits for this branch
      const { data: visits } = await supabase
        .from('visits')
        .select('visit_date, pest_types, notes')
        .eq('branch_id', branch.id)
        .order('visit_date', { ascending: false })
        .limit(24);
      ...
      visits?.forEach(v => {
        visitCount++;
        v.pest_types?.forEach((p: string) => { pestCounts[p] = (pestCounts[p] || 0) + 1; });
      });
      const topPests = Object.entries(pestCounts).sort((a, b) => b[1] - a[1]).slice(0, 5);
      setVisitStats({ count: visitCount, pests: topPests.map(([p]) => p) });

      const customerContext = customer ? `
        MüŞTERİ / İŞLETME BİLGİSİ:
        - Firma Adı: ${customer.cari_isim || customer.kisa_isim}
        - Kısa İsim: ${customer.kisa_isim}
        - Şehir: ${customer.sehir || 'Belirtilmemiş'}
        TEMEL GÖREV: Firma ismine bakarak işletmenin sektörünü (otel, fabrika, restoran, market, okul, depo, hastane vb.) tahmin et. Bu sektörün haşere risk profiline etkisini analize dahil et.` : '';

      const prompt = `
        Aşağıdaki şubeye özel haşere kontrol verilerini analiz et ve kapsam lı Türkçe risk raporu hazırla.
        ${customerContext}

        ŞUBE BİLGİLERİ:
        - Şube Adı: ${branch.sube_adi}
        - Şehir/Adres: ${branch.sehir || 'Belirtilmemiş'}, ${branch.adres || ''}
        - Koordinat: ${branch.latitude ? `${branch.latitude}, ${branch.longitude}` : 'Koordinat kayıtsız'}

        SON 12 AY VERİLERİ:
        - Toplam Ziyaret: ${visitCount}
        - En Çok Tespit Edilen Zararlılar: ${topPests.map(([p, c]) => `${p} (${c} kez)`).join(', ') || 'Tespit edilmedi'}

        LÜTFEN SİRASIYLA ANALİZ ET (başlıklı maddeler halinde):

        ## 1. İşletme Sektörü ve Risk Profili
        Firma ismine göre sektörü tahmin et. Bu sektörün tipik haşere risk faktörlerini açıkla.

        ## 2. Lokasyon ve İklim Analizi
        Koordinatlar ve şehir itibarıyla iklim koşullarını değerlendir.

        ## 3. Mevcut Zararlı Aktivitesi
        Tespit edilen zararlıların bu lokasyon ve sektörle bağlantısını açıkla.

        ## 4. Öngörülen Riskler (Nisan-Haziran)
        Önümüzdeki 3 ay için mevsimsel risk artışlarını belirt.

        ## 5. Stratejik Aksiyon Planı
        Bu şubeye ve sektöre özel 4-5 somut önlem öner.

        ## 6. Ziyaret ve İlaçlama Önerisi
        Optimal ziyaret sıklığı ve ilaçlama türü önerisi ver.

        Yanıtı profesyonel Türkçe ile ver.
      `;

        // Retry logic: up to 3 attempts for 429 / 503 transient errors
        const MAX_RETRIES = 3;
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          ...
          if (resp.status === 429 || resp.status === 503) {
            ...
            lastErr = resp.status === 429 ? 'İstek sınırı' : 'Sunucu yoğunluğu';
            if (attempt < MAX_RETRIES) {
              setAnalysis(`⏳ ${lastErr} — ${waitSec} saniye sonra yeniden deneniyor... (${attempt}/${MAX_RETRIES})`);
              await new Promise(r => setTimeout(r, waitSec * 1000));
              continue;
            }
            throw new Error(`${lastErr}: ${MAX_RETRIES} denemede başarısız. Lütfen birkaç dakika sonra tekrar deneyin.`);
          }
          break; // success
        }
      ...
      if (text) {
        setAnalysis(text);
        setVisitStats({ count: visitCount, pests: topPests.map(([p]) => p) });
        // Auto-save: check if existing record, then update or insert
        setSaving(true);
        try {
          const pestArray = topPests.map(([p]) => p);
          // Try update first (works if table exists + row exists)
          const { data: existing } = await supabase
            .from('ai_branch_analyses')
            .select('id')
            .eq('branch_id', branch.id)
            .maybeSingle();
          ...
          // Table may not exist yet — show in console + alert
          console.error('Analiz kaydedilemedi:', saveErr);
          alert(`⚠ Analiz kaydedilemedi: ${saveErr?.message || 'Tablo mevcut olmayabilir. Supabase SQL Editor\'dan migration dosyasını çalıştırın.'}`);
        } finally {
          setSaving(false);
        }
  ...
  const handlePrint = async () => {
    if (!analysis) return;
    const { printBranchAiReport } = await import('../utils/branchAiReportGenerator');
    printBranchAiReport({
      branch,
      customer: customer || { kisa_isim: branch.sube_adi },
      settings: companySettings,
      analysisText: analysis,
      visitCount: visitStats.count,
      topPests: visitStats.pests,
    });
  };
  ...
  return (
    <div className="space-y-4">
      {/* Action bar */}
      <div className="flex items-center justify-between gap-4 p-4 bg-purple-50 border border-purple-100 rounded-xl">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 bg-purple-100 rounded-xl flex items-center justify-center">
            <Brain className="text-purple-600" size={18} />
          </div>
          <div>
            <p className="font-bold text-purple-900 text-sm">İLAÇLAMATİK YAPAYZEKA Şube Analizi</p>
            <p className="text-xs text-purple-600">
              {branch.sube_adi}{customer && <span className="ml-1 text-purple-400">· {customer.kisa_isim}</span>}{' '}— Sektör + Lokasyon + Zararlı Verisi
            </p>
          </div>
        </div>
        <div className="flex gap-2 flex-wrap justify-end">
          {saving && (
            <span className="flex items-center gap-1 text-xs text-emerald-600 font-medium">
              <RefreshCw size={11} className="animate-spin" /> Kaydediliyor...
            </span>
          )}
          {analysis && (
            <button
          ...
          <button
            onClick={runAnalysis}
            disabled={analyzing}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-600 text-white text-xs font-bold rounded-lg hover:bg-purple-700 transition-all disabled:opacity-50"
          >
            {analyzing ? <RefreshCw size={13} className="animate-spin" /> : <Brain size={13} />}
            {analyzing ? 'Analiz Ediliyor...' : analysis ? 'Yenile' : 'Analizi Başlat'}
          </button>
        </div>
      </div>
      ...
      {/* Result */}
      {analyzing && (
        <div className="py-10 flex flex-col items-center gap-3 text-purple-500">
          <RefreshCw className="animate-spin" size={28} />
          <p className="text-sm font-medium">İLAÇLAMATİK YAPAYZEKA şube verileri analiz ediyor...</p>
        </div>
      )}
      {!analyzing && analysis && (
        <div className="bg-white border border-purple-100 rounded-xl p-5">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-bold text-purple-500 uppercase tracking-wider">AI Analiz Sonucu</p>
            {savedRecord && (
              <span className="flex items-center gap-1 text-xs text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">
                <CheckCircle size={11} /> Kaydedildi: {format(parseISO(savedRecord.updated_at || savedRecord.created_at), 'dd.MM.yyyy HH:mm', { locale: tr })}
              </span>
            )}
          </div>
          <div className="text-sm text-gray-700 leading-relaxed whitespace-pre-line">{analysis}</div>
        </div>
      )}
      {!analyzing && !analysis && (
        <div className="py-10 border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center text-center">
          <Brain className="w-10 h-10 text-gray-200 mb-3" />
          <p className="text-sm font-medium text-gray-500">Analiz henüz çalıştırılmadı</p>
          <p className="text-xs text-gray-400 mt-1 max-w-xs">Bu şubeye özel AI analizi için yukarıdaki butona tıklayın.</p>
          {!branch.latitude && (
            <p className="mt-3 text-xs text-amber-600 bg-amber-50 px-3 py-1.5 rounded-lg">⚠ Koordinat eksik — şube haritasına konum eklenirse analiz daha isabetli olur.</p>
          )}
        </div>
      )}
    </div>
            ...
            return (
              <div key={branch.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div className="bg-gray-50 px-4 py-2 border-t border-gray-200 flex flex-wrap gap-2">
                  ...
                  <button onClick={() => toggleTab(branch.id, 'equipment-map')} className={`flex items-center gap-2 px-3 py-2 text-sm font-medium rounded-md transition-colors ${activeTab === 'equipment-map' ? 'bg-white text-blue-600 shadow-sm' : 'text-gray-600 hover:bg-white'}`}><MapIcon size={16} /> Ekipman Haritası {activeTab === 'equipment-map' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>
                  ...
                  <button onClick={() => toggleTab(branch.id, 'ai-analysis')} className={`flex items-center gap-2 px-3 py-2 text-sm font-bold rounded-md transition-colors ${activeTab === 'ai-analysis' ? 'bg-purple-600 text-white shadow-sm' : 'text-purple-600 hover:bg-purple-50'}`}><Brain size={16} /> AI Analiz {activeTab === 'ai-analysis' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button>
                </div>
                {activeTab && (
                  <div className="border-t border-gray-200 p-4 bg-gray-50/30 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    {activeTab === 'equipment-map' && <BranchEquipmentMapView branchId={branch.id} branchData={branch} />}
                    ...
                    {activeTab === 'env-risk' && <BranchEnvironmentalRiskView branchId={branch.id} branchData={branch} />}
                    {activeTab === 'ai-analysis' && <BranchAiAnalysisView branch={branch} />}
                  </div>
                )}
              </div>
            );
