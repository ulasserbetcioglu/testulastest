...
interface CompanySettings {
  ...
  parasut_invoice_note?: string; // NEW
}
        ...
        .insert([
          {
            ...
            footer_text: '',
            logo_url: '',
            parasut_invoice_note: 'MÜŞTERİ GÜNCEL BAKİYESİ: {BALANCE} TL\n\nÖDEME BİLGİLERİ:\nBanka: Garanti BBVA\nIBAN: TR66 0006 2000 0370 0006 2027 89'
          }
        ]);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Google Review Linki (Anket Sonu İçin)
            </label>
            <input
              type="text"
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Paraşüt Fatura Notu (Sabit Metin)
            </label>
            <p className="text-xs text-gray-400 mb-2">
              Paraşüt'e aktarılan her faturanın sonuna bu metin eklenir. 
              Giriş: <code className="bg-gray-100 px-1">{'{BALANCE}'}</code> etiketi otomatik olarak müşteri bakiyesi ile değiştirilir.
            </p>
            <textarea
              value={settings.parasut_invoice_note || ''}
              onChange={(e) => setSettings({ ...settings, parasut_invoice_note: e.target.value })}
              rows={5}
              className="w-full p-2 border rounded focus:ring-2 focus:ring-green-500 disabled:bg-gray-100 font-mono text-sm"
              disabled={!isAdmin}
              placeholder="Fatura notu metni..."
            />
          </div>
        </div>
      </div>
