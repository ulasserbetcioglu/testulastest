...
import { Search, Download, Eye, RefreshCw, AlertTriangle, ChevronLeft, ChevronRight } from 'lucide-react';
...
const CustomerPaidMaterials: React.FC = () => {
  ...
  }, [customerId, isAdmin, debouncedSearchTerm, selectedMonth, startDate, endDate, currentPage]);

  const fetchCustomerId = async () => {
    try {
      const id = await localAuth.getCurrentCustomerId();
      if (!id) {
        // Eğer localAuth'da müşteri ID'si yoksa, Supabase Auth üzerinden admin olup olmadığını kontrol et
  ...
  const fetchSales = useCallback(async () => {
    try {
      ...
      if (debouncedSearchTerm) {
        const filters = [
          `branch:branch_id.sube_adi.ilike.%${debouncedSearchTerm}%`,
          `items.product:product_id.name.ilike.%${debouncedSearchTerm}%`
        ];
        
        if (isAdmin) {
          filters.push(`customer:customer_id.kisa_isim.ilike.%${debouncedSearchTerm}%`);
        }
      ...
      // Supabase bazen ilişkili verileri dizi olarak dönebilir, bunları nesneye çevirelim
      const formattedData = (data as any[] || []).map(sale => ({
        ...sale,
        ...
        visit: Array.isArray(sale.visit) ? sale.visit[0] : sale.visit
      }));
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
        <h2 className="text-2xl font-semibold text-gray-800">
          {isAdmin ? 'TÜM ÜCRETLİ MALZEME SATIŞLARI' : 'ÜCRETLİ MALZEME SATIŞLARIM'}
        </h2>
        <button
      ...
      <div className="bg-white rounded-lg shadow p-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-4">
          <div className="flex-1 relative">
            <input
              type="text"
              placeholder={isAdmin ? "Müşteri, Şube veya Ürün Ara..." : "Şube veya Ürün Ara..."}
              value={searchTerm}
