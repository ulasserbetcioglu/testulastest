...
import { FileText, Download, Eye, Calendar, Filter, X, Search, Building2 } from 'lucide-react';
...
const AdminModuleReports: React.FC = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      let riskQuery = supabase
        ...
        .select('id, assessment_date, report_url, client_company, status, customer_id, branch_id')
        .order('assessment_date', { ascending: false });

      if (selectedCustomerId) {
        riskQuery = riskQuery.eq('customer_id', selectedCustomerId);
      }
      ...
      let proposalQuery = supabase
        .from('proposals')
        .select('id, proposal_date, report_url, company_name, status, customer_id, branch_id')
        .order('proposal_date', { ascending: false });

      if (selectedCustomerId) {
        proposalQuery = proposalQuery.eq('customer_id', selectedCustomerId);
      }
      ...
      let uvQuery = supabase
        .from('uv_lamp_reports')
        .select('id, report_date, report_url, customer_name, location, status, customer_id, branch_id')
        .order('report_date', { ascending: false });

      if (selectedCustomerId) {
        uvQuery = uvQuery.eq('customer_id', selectedCustomerId);
      }

      const { data: uvData, error: uvError } = await uvQuery;
  ...
  const filteredReportsBySearch = reports.filter(report =>
  ...
  return (
    <div className="p-6">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Modül Raporları Yönetimi</h1>
        <p className="text-gray-600">Tüm müşterilere ait modül raporlarını görüntüleyin ve yönetin</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Müşteri veya şube adına göre ara..."
              value={searchTerm}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            Tümü ({filteredReportsBySearch.length})
          </button>
          <button
      ...
      {filteredReportsBySearch.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <p className="text-gray-600">
            {selectedType === 'all'
              ? 'Henüz rapor oluşturulmamış veya arama kriterlerine uygun rapor bulunamadı.'
            ...
            <div className="flex items-center justify-between p-4 border-b">
              <div>
                ...
                {selectedReport.customer_name && (
                  <p className="text-sm text-gray-500">{selectedReport.customer_name}</p>
                )}
              </div>
              <button
...
export default AdminModuleReports;
