...
import { Plus, FileText, Download, Upload, X, CheckCircle, Clock, Eye, Edit, Trash, Image as ImageIcon, ClipboardCheck } from 'lucide-react';
...
const AdminVisits: React.FC = () => {
  ...
  const [selectedReportImageFilePath, setSelectedReportImageFilePath] = useState<string>('');
  ...
  const fetchData = async () => {
    try {
      ...
      while (hasMore) {
        ...
        if (data) {
          allVisitsData = allVisitsData.concat(data as any);
          offset += data.length;
      ...
      const enhancedVisits = (allVisitsData || []).map(visit => {
        ...
        return {
          ...visit,
          customer: Array.isArray(anyVisit.customer) ? anyVisit.customer[0] : anyVisit.customer,
          branch: Array.isArray(anyVisit.branch) ? anyVisit.branch[0] : anyVisit.branch,
          operator: Array.isArray(anyVisit.operator) ? anyVisit.operator[0] : anyVisit.operator,
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const updatedVisit: Visit = {
        ...editingVisit,
        visit_date: visitDateTime,
        status: editFormData.status as 'planned' | 'completed' | 'cancelled',
        visit_type: editFormData.visitType,
        notes: editFormData.notes,
        report_number: editFormData.reportNumber
      };
  ...
  const handleViewReportImage = (filePath: string, reportNumber?: string) => {
    setSelectedReportImageFilePath(filePath);
    setSelectedReportImageTitle(reportNumber ? `Rapor Fotoğrafı: ${reportNumber}` : 'Rapor Fotoğrafı');
    setShowReportImageModal(true);
  };

  const handleGenerateEk1Report = async (visit: Visit) => {
    try {
      setLoading(true);
      
      // 1. Şirket ayarlarını çek
      ...
      // 2. Operatör detaylarını çek (sertifika bilgisi için)
      const { data: operatorData } = await supabase
        .from('operators')
        .select('*')
        .eq('id', visit.operator_id)
        .single();

      // 3. Biyosidal ürün kullanımını çek
      ...
      // 4. Müşteri adresini çek (Eğer ziyaret objesinde yoksa)
      let customerAddress = '';
      if (visit.customer_id) {
         const { data: cData } = await supabase
           ...
           .single();
         customerAddress = cData?.adres || '';
      }

      const biocidalUsageItems = (usageData || []).map((u: any) => ({
        name: u.product?.name || 'Bilinmeyen Ürün',
        licenseNumber: u.product?.license_number || '-',
        manufacturer: u.product?.manufacturer || '-',
        activeIngredient: u.product?.active_ingredient || '-',
        batchNumber: '', // Opsiyonel
  ...
  const columns = [
    ...
    {
      header: 'Fotoğraf',
      accessor: 'report_photo_file_path' as keyof Visit,
      render: (_: string | undefined, row: Visit) => (
        <div className="flex justify-center">
          {row.report_photo_file_path ? (
            <button
              onClick={() => handleViewReportImage(row.report_photo_file_path!, row.report_number || row.id)}
    ...
    <div className="container mx-auto px-2 py-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        <h1 className="text-lg sm:text-xl font-bold flex items-center gap-2">
          Ziyaret Yönetimi
          {loading && <span className="text-xs font-normal text-blue-500">Yükleniyor...</span>}
          {error && <span className="text-xs font-normal text-red-500">Hata: {error}</span>}
        </h1>
        ...
        <DataTable
          ...
          actions={(visit) => (
            <div className="flex justify-end space-x-2">
              ...
              <button
                onClick={() => handleGenerateEk1Report(visit)}
                className="text-orange-600 hover:text-orange-900"
                title="EK-1 Raporu (PDF)"
      ...
      <ReportImageModal 
        ...
        filePath={selectedReportImageFilePath}
