...
const FloorPlanViewer: React.FC<FloorPlanViewerProps> = ({ branchId }) => {
  ...
  const activeFloorData = useMemo(() => {
    ...
    // Varsayılan boyutlar (Veritabanında yoksa kullanılacak)
    ...
    if (currentPlan.floors && currentPlan.floors.length > 0) {
      ...
      return {
        ...
        name: floor.name,
        // DÜZELTME: Sadece veritabanı boyutlarını kullan, resim boyutunu bekleme.
  ...
  const handleWheel = (e: React.WheelEvent) => {
    // Sadece kroki üzerindeyken zoom yap
  ...
  return (
    <div className="space-y-4">
      ...
      >
        ...
        >
          {/* viewBox, boyutları veritabanından gelen değerlere sabitler */}
          <svg 
            width={activeFloorData?.width}
            height={activeFloorData?.height}
          ...
          >
            ...
            {activeFloorData?.background ? (
              <image 
                ...
                width={activeFloorData.width}
                height={activeFloorData.height}
                // DÜZELTME: preserveAspectRatio="none" resmi koordinat düzlemine tam oturtur. 
