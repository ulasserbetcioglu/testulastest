...
const AddEquipmentModal: React.FC<AddEquipmentModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (branchId) {
        fetchExistingDepartments(branchId);
      }
    }
  }, [isOpen, customerId, initialDepartment, branchId]);

  const fetchExistingDepartments = async (bId: string) => {
    try {
  ...
  const fetchBranches = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        fetchExistingDepartments(data[0].id);
      }
    } catch (err: any) {
      setError(err.message);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Fetch existing equipment to determine next numbers
      const { data: existingEquipment, error: fetchError } = await supabase
        .from('branch_equipment')
        .select('equipment_code, equipment_id')
      ...
      const equipmentToInsert = formData.items.flatMap(item => {
        ...
        const prefix = selectedEquipment.code;

        // Find the highest number for this equipment type in the whole branch
        if (nextNumbers[item.equipmentId] === undefined) {
          const codes = (existingEquipment || [])
            .filter(ee => ee.equipment_id === item.equipmentId)
            .map(ee => {
              const parts = ee.equipment_code.split('-');
              const num = parseInt(parts[parts.length - 1]);
              return isNaN(num) ? 0 : num;
            });

          nextNumbers[item.equipmentId] = Math.max(0, ...codes) + 1;
        }

        const inserts = Array.from({ length: item.count }, () => {
          const currentNum = nextNumbers[item.equipmentId];
          nextNumbers[item.equipmentId]++;
          return {
            branch_id: targetBranchId,
            equipment_id: item.equipmentId,
            equipment_code: `${prefix}-${currentNum}`,
