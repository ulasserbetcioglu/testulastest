...
const PestActivityPreview: React.FC<Props> = ({ report, companySettings, compact }) => {
  ...
  const handlePrint = () => {
    ...
    <style>
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      td, th { padding: 4px 6px; border: 1px solid #d1d5db; font-size: 9px; vertical-align: top; }
  ...
  const handleExportPdf = async () => {
    ...
    try {
        ...
        // --- TABLO VERİSİ HAZIRLAMA (ROWSPAN İLE) ---
        const tableBody: any[] = [];
        ...
        pestRows.forEach((pest, index) => {
            ...
            const limitCount = pest.limits && pest.limits.length > 0 ? pest.limits.length : 1;

            if (pest.limits && pest.limits.length > 0) {
                pest.limits.forEach((limit, lIdx) => {
                    // Renk Ayarı
                    let cellColor = '#ffffff'; 
                    if (limit.level === 'kabul') cellColor = '#dcfce7'; 
                    else if (limit.level === 'aktivite') cellColor = '#fef3c7'; 
                    else if (limit.level === 'istila') cellColor = '#fecaca'; 

                    const limitContent = `${toAscii(LEVEL_LABELS[limit.level] || limit.level).toUpperCase()}: ${toAscii(limit.description)}`;

                    if (lIdx === 0) {
                        // GRUBUN İLK SATIRI: Ana hücreler burada oluşturulur ve rowSpan verilir
                        tableBody.push([
    ...
    <div>
      {!compact && (
        <div className="flex gap-2 mb-4 justify-end print:hidden">
          ...
          <button onClick={handleExportPdf} disabled={isGenerating} className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:opacity-50">
            {isGenerating ? <Loader2 size={16} className="animate-spin" /> : <FileDown size={16} />} PDF Indir (Vektorel)
          </button>
        </div>
      )}

      <div ref={reportRef} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        {/* Görsel Önizleme (HTML Tablo - RowSpan Kullanılarak Düzenlendi) */}
              ...
              {pestRows.map((pest, idx) => {
                ...
                return pest.limits.map((limit, lIdx) => (
                  <tr key={`${idx}-${lIdx}`} className={idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}>
                    {lIdx === 0 && (
                      <>
                        <td className="border border-gray-300 px-3 py-2 text-center font-bold text-gray-700 align-middle" rowSpan={limitCount}>
                          {idx + 1}
                        </td>
                        <td className="border border-gray-300 px-3 py-2 font-semibold text-gray-800 align-middle" rowSpan={limitCount}>
                          {pest.pest_name}
                        </td>
                        <td className="border border-gray-300 px-3 py-2 text-center text-gray-600 font-medium align-middle" rowSpan={limitCount}>
                    ...
                    <td className="border border-gray-300 px-3 py-1.5 align-middle">
                    ...
                    {lIdx === 0 && (
                      <td className="border border-gray-300 px-3 py-2 text-gray-600 align-middle leading-relaxed" rowSpan={limitCount}>
