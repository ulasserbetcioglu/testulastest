    ...
    const load = async () => {
      ...
      try {
        ...
        if (branchId) {
          ...
          if (branchData) {
            // @ts-ignore
            const c = Array.isArray(branchData.customers) ? branchData.customers[0] : branchData.customers;
            // No longer using branchName/mainCustomerName in preview props as they are not accepted
          }
        }
