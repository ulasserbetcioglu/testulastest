...
const BranchActionPlanView: React.FC<Props> = ({ branchId, readOnly = false }) => {
    ...
    const fetchHeaderData = async () => {
        try {
            ...
            const { data: branchData } = await supabase
                .from('branches')
                .select('id, sube_adi, adres, sehir, authorized_person, customers(id, kisa_isim, cari_isim, adres, sehir)')
                .eq('id', branchId)
                .maybeSingle();
            ...
            if (branchData) {
                ...
                setHeaderData({
                    ...
                    customerAddress: branchData.adres || cust?.adres || '',
                    customerCity: branchData.sehir || cust?.sehir || '',
    ...
    const handleDeletePlan = async () => {
        if (!window.confirm('BU ŞUBEYE AİT TÜM ACİL EYLEM PLANINI SİLMEK İSTEDİĞİNİZE EMİN MİSİNİZ? Bu işlem geri alınamaz!')) return;
        ...
        try {
            ...
            toast.success('Acil eylem planı başarıyla silindi');
            setLimits([]);
            setEditingId(null);
            setNewLimit({});
        } catch (error: any) {
            console.error('Error deleting plan:', error);
            toast.error('Plan silinirken hata oluştu: ' + error.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    const downloadPDF = async () => {
        ...
        if (!currentHeader) {
            ...
            const { data: branchData } = await supabase
                .from('branches')
                .select('id, sube_adi, adres, sehir, customers(id, kisa_isim, cari_isim, adres, sehir)')
                .eq('id', branchId)
                .maybeSingle();
            ...
            if (branchData) {
                ...
                currentHeader = {
                    ...
                    responsiblePerson: 'PestMentor',
                    // @ts-ignore
                    customerResponsible: branchData.authorized_person || '',
                    companyLogo: companySettings?.logo_url || null
                };
        ...
        try {
            ...
            await generateActionPlanPdf({
                ...
                customerResponsible: currentHeader.customerResponsible,
                        ...
                        <>
                            {limits.length === 0 ? (
                                ...
                                <button
                                    ...
                                    className="flex items-center gap-2 px-3 py-2 bg-white border border-red-200 text-red-600 rounded-lg text-sm font-medium hover:bg-red-50"
                                    title="Planı Sil"
                                >
                                    <Trash2 size={16} />
                                    <span className="hidden sm:inline">Planı Sil</span>
                                </button>
                            )}
                            <button
