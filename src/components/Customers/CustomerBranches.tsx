...
import { Plus, Pencil, Trash2, Download, Upload, Layout, Eye, ChevronDown, ChevronUp } from 'lucide-react';
import { supabase } from '../../lib/supabase';
import type { Branch } from '../../types';
import AddBranchModal from './AddBranchModal';
import EditBranchModal from './EditBranchModal';
import BranchEquipment from '../Branches/BranchEquipment';
import FloorPlanViewer from '../Branches/FloorPlanViewer';
...
const CustomerBranches: React.FC<CustomerBranchesProps> = ({ customerId }) => {
  ...
  const [activeTabMapping, setActiveTabMapping] = useState<Record<string, 'equipment' | 'floorplan' | null>>({});
          ...
          {branches.map((branch) => (
            ...
            >
              ...
              <div className="border-t flex divide-x">
                <button
                  onClick={() => {
                    const current = activeTabMapping[branch.id];
                    setActiveTabMapping(prev => ({
                      ...prev,
                      [branch.id]: current === 'equipment' ? null : 'equipment'
                    }));
                  }}
                  className={`flex-1 px-4 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTabMapping[branch.id] === 'equipment' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                  <Eye size={16} />
                  Ekipmanlar {activeTabMapping[branch.id] === 'equipment' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                </button>
                <button
                  onClick={() => {
                    const current = activeTabMapping[branch.id];
                    setActiveTabMapping(prev => ({
                      ...prev,
                      [branch.id]: current === 'floorplan' ? null : 'floorplan'
                    }));
                  }}
                  className={`flex-1 px-4 py-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTabMapping[branch.id] === 'floorplan' ? 'bg-green-50 text-green-700' : 'text-gray-600 hover:bg-gray-50'}`}
                >
                  <Layout size={16} />
                  Krokiyi Göster {activeTabMapping[branch.id] === 'floorplan' ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
                </button>
              </div>

              {activeTabMapping[branch.id] === 'equipment' && (
                <div className="border-t p-4 bg-gray-50/30 animate-in fade-in slide-in-from-top-2 duration-200">
                  <BranchEquipment branchId={branch.id} />
                </div>
              )}

              {activeTabMapping[branch.id] === 'floorplan' && (
                <div className="border-t p-4 bg-gray-50/30 animate-in fade-in slide-in-from-top-2 duration-200">
                  <div className="h-[600px] rounded-lg border bg-white overflow-hidden">
                    <FloorPlanViewer branchId={branch.id} />
                  </div>
                </div>
              )}
            </div>
