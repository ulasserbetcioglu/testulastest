...
const EditBranchModal: React.FC<EditBranchModalProps> = ({ isOpen, onClose, branch, onSave }) => {
  const [formData, setFormData] = useState({
    subeAdi: branch.sube_adi,
    yetkiliAdi: branch.authorized_person || '',
    adres: branch.adres || '',
    sehir: branch.sehir || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updateData: any = {
        ...
        authorized_person: formData.yetkiliAdi,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-2xl">
        <div className="flex justify-between items-center p-4 border-b">
          <h2 className="text-xl font-semibold">Şube Düzenle</h2>
