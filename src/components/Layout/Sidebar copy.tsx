...
import {
  ...
  FileText, LogOut,
  Users, User, Settings, UserCog, Route, Building, Home, CalendarClock, MapPin,
  ChevronsLeft, ChevronsRight, ChevronDown,
  Mail, Package, MessageSquare, MailCheck,
  BarChart3, TrendingUp, PlusCircle,
  Image as ImageIcon, Clock as ClockIcon, Car, Bug, Presentation, Shield, AlertTriangle, RefreshCw, Brain
} from 'lucide-react';
...
const menuCategories = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Toplu Ziyaret', path: '/bulk-visit-import', icon: <PlusCircle size={18} />, adminOnly: true },
    ]
  },
  {
    title: 'SATIŞ & SÖZLEŞME',
    items: [
      ...
      { name: 'Fiyat Artışı', path: '/fiyat-artisi', icon: <TrendingUp size={18} /> },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Eksik Ziyaretler', path: '/admin/eksik-ziyaretler', icon: <CalendarClock size={18} /> },
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Acil Eylem Planı', path: '/acil-eylem-plani', icon: <Shield size={18} /> },
      { name: 'Risk Değerlendirme', path: '/zararli-risk-degerlendirme', icon: <Bug size={18} /> },
      { name: 'Çevre Risk Değ.', path: '/environmental-risk-assessment', icon: <Bug size={18} /> },
      { name: 'Onaylı Pestisit Listesi', path: '/onayli-pestisit-listesi', icon: <Bug size={18} /> },
      { name: 'DÖF', path: '/dof', icon: <ImageIcon size={18} /> },
    ]
  },
  {
    title: 'İK & ARAÇ YÖNETİMİ',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Mutabakat', path: '/admin/mutabakat', icon: <RefreshCw size={18} /> },
      { name: 'Müşteri Kayıtları', path: '/musteri-kayitlari', icon: <User size={18} /> },
    ]
  },
  {
    title: 'PAZARLAMA & İLETİŞİM',
    items: [
      ...
      { name: 'Bilgilendirme Paz.', path: '/bilgilendirme-pazarlama', icon: <MessageSquare size={18} /> },
      { name: 'E-Posta Paz.', path: '/eposta-pazarlama', icon: <Mail size={18} /> },
      { name: 'Anket Sonuçları', path: '/anket-sonuclari', icon: <BarChart2 size={18} /> },
      { name: 'E-postalar', path: '/gonderilen-epostalar', icon: <MailCheck size={18} /> },
    ]
  },
  {
    title: 'VARLIK YÖNETİMİ',
    items: [
      { name: 'Depolar', path: '/depolar', icon: <Warehouse size={18} /> },
      { name: 'Ekipmanlar', path: '/ekipman-yonetimi', icon: <Package size={18} /> },
      { name: 'Hizmet Yönetimi', path: '/hizmet-yonetimi', icon: <MessageSquare size={18} /> },
      { name: 'Eğitim Sunum.', path: '/egitim-sunumlari', icon: <Presentation size={18} /> },
    ]
  },
  {
    title: 'RAPORLAMA & ANALİZ',
    items: [
      ...
      { name: 'Tahminleme', path: '/tahminleme', icon: <Brain size={18} /> },
      { name: 'Faaliyet Rap.', path: '/faaliyet-rapor-takip', icon: <FileText size={18} /> },
      { name: 'Pestisit', path: '/pestisit-raporu', icon: <Bug size={18} /> },
      { name: 'Faaliyet Dos. Yön.', path: '/admin/faaliyet-dosyasi-yonetimi', icon: <FileText size={18} /> },
    ]
  },
  {
    ...
    items: [
      ...
      { name: 'Paraşüt Eşleştirme', path: '/admin/parasut-eslestirme', icon: <RefreshCw size={18} />, adminOnly: true },
      { name: 'Paraşüt Ayarları', path: '/admin/parasut-ayarlari', icon: <Settings size={18} />, adminOnly: true },
    ]
  }
];
          ...
          {menuCategories.map((category) => {
            ...
            return (
              <div key={category.title} className="mb-3">
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${(isExpanded || isCollapsed) ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'
                  }`}>
