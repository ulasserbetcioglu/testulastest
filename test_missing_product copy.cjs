...
async function run() {
    console.log('Searching for "DEĞİRMEN" with filter[query]...');
    const res = await postData({
        type: 'products',
        query: 'DEĞİRMEN',
        filter_key: 'filter[query]'
    });
    console.log(`Results: ${res.data?.length || 0}`);
    if (res.data) {
        res.data.forEach(i => console.log(`- ${i.name} (Code: ${i.code})`));
    }

    console.log('\nSearching for "DEGIRMEN" (Latin) with filter[query]...');
